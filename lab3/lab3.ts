...
export function kjbEmbed(
  ...
  for (let row = crossPixelCount; row < modifiedContainer.getHeight() - crossPixelCount; row+=crossPixelCount) {
    for (let column = crossPixelCount; column < modifiedContainer.getWidth() - crossPixelCount; column+=crossPixelCount) {
      ...
      let modifiedB =  modifiedBTemp > 0 ? Math.min(modifiedBTemp, 255) : Math.max(modifiedBTemp, 0)
      //-------
...
export function kjbExtract(
  ...
  for (let row = crossPixelCount; row < container.getHeight() - crossPixelCount; row+=crossPixelCount) {
    for (let column = crossPixelCount; column < container.getWidth() - crossPixelCount; column+=crossPixelCount) {
      ...
      let blueEstimated = estimation / (4 * crossPixelCount)
      //--------------------------------
